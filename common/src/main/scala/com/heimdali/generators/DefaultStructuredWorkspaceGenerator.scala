...
class DefaultStructuredWorkspaceGenerator[F[_]](appConfig: AppConfig)
  ...
  override def ldapConfig: LDAPConfig = appConfig.ldap
  ...
  def db(dataset: String, generatedName: String, structuredTemplate: StructuredTemplate): F[HiveAllocation] =
    for {
      manager <- generate(
  ...
  override def workspaceFor(structuredTemplate: StructuredTemplate): F[WorkspaceRequest] = {
    ...
    for {
      raw <- db("raw", generatedName, structuredTemplate)
      staging <- db("staging", generatedName, structuredTemplate)
      modeled <- db("modeled", generatedName, structuredTemplate)
