...
class ComplianceGroupServiceSpec extends FlatSpec with Matchers with DBTest with MockFactory with AppContextProvider {
  ...
  it should "update an existing compliance group" in new Context {
    ...
    val updatedComplianceGroup = savedComplianceGroup.copy(questions = List(savedComplianceGroup.questions.get(0).get))
    ...
    context.complianceQuestionRepository.create _ expects (1L, savedComplianceGroup.questions.get(0).get) returning 1L.pure[ConnectionIO]
