...
trait AkkaModule {
  ...
  val sharedWorkspaceSaver: ActorRef = actorSystem.actorOf(WorkspaceSaver[SharedWorkspace](sharedWorkspaceRepository, ldapRepository, hiveDatabaseRepository))
  val userWorkspaceSaver: ActorRef = actorSystem.actorOf(WorkspaceSaver[UserWorkspace](userWorkspaceRepository, ldapRepository, hiveDatabaseRepository))
  val datasetWorkspaceSaver: ActorRef = actorSystem.actorOf(WorkspaceSaver[Dataset](datasetRepository, ldapRepository, hiveDatabaseRepository))
  val hdfsActor: ActorRef = actorSystem.actorOf(HDFSActor.props(hdfsClient, loginContextProvider, configuration))
