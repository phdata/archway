...
class UserGeneratorTest extends PropSpec with TableDrivenPropertyChecks with Matchers {
  ...
  val userInput =
    Table(
      ("input", "output"),
      (UserTemplate("johndoe", Some(1), Some(1), Some(1)), WorkspaceRequest("johndoe", "johndoe", "johndoe", "user", "johndoe", Instant.now(), Compliance(phiData = false, pciData = false, piiData = false), singleUser = true, data = List(HiveDatabase("user_johndoe", "/user/johndoe/db", 250, 0, LDAPRegistration("cn=user_johndoe,ou=groups,ou=hadoop,dc=jotunn,dc=io", "user_johndoe", "role_user_johndoe"), None)), processing = List(Yarn("root.user.johndoe", 1, 1)))),
      (UserTemplate("johndoe", Some(1), None, None), WorkspaceRequest("johndoe", "johndoe", "johndoe", "user", "johndoe", Instant.now(), Compliance(phiData = false, pciData = false, piiData = false), singleUser = true, data = List(HiveDatabase("user_johndoe", "/user/johndoe/db", 250, 0, LDAPRegistration("cn=user_johndoe,ou=groups,ou=hadoop,dc=jotunn,dc=io", "user_johndoe", "role_user_johndoe"), None)))),
