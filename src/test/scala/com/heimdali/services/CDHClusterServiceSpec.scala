...
class CDHClusterServiceSpec
  ...
  it should "return a cluster" in {
    ...
    val clusterRequest = Get("/clusters/cluster%20name").addCredentials(BasicHttpCredentials(username, password))
    (http.request _).expects(clusterRequest).returning(Marshal(clusterJson).to[HttpResponse])
    val impalaRequest = Get("/clusters/cluster%20name/services/impala/roles").addCredentials(BasicHttpCredentials(username, password))
    ...
    val hiveRequest = Get("/clusters/cluster%20name/services/hive/roles").addCredentials(BasicHttpCredentials(username, password))
    ...
    val servicesRequest = Get("/clusters/cluster%20name/services").addCredentials(BasicHttpCredentials(username, password))
