    ...
    (server)
        case ${CMD} in
            (start)
                ...
                FLYWAY_COMMAND="filesystem:$SCRIPTS_LOCATION $FLYWAY_EXECUTABLE migrate -url=$DB_URL -user=$DB_USERNAME -password="
                set +x
                env FLYWAY_LOCATIONS=${FLYWAY_COMMAND}$DB_PASSWORD || exit 1
                set -x
