...
class AccountControllerSpec
  ...
  it should "return not found if a personal workspace hasn't been created yet" in new Context {
    accountService.getWorkspace _ expects standardUserDN.value returning OptionT.none
  ...
  it should "return the workspace if one exists" in new Context {
    accountService.getWorkspace _ expects standardUserDN.value returning OptionT.some(savedWorkspaceRequest)
